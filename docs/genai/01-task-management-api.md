...
A representative sample of what the model produced (trimmed):
...
public class TaskService : ITaskService
{
    ...
    public async Task<TaskItem> CreateAsync(CreateTaskRequest req, Guid userId)
    {
...
|---|---|---|
| `enum TaskStatus` collides with `System.Threading.Tasks.Task` naming and shadows the BCL `TaskStatus`. | Compiles but confusing and error-prone; `Task<TaskItem>` vs `TaskStatus` is a readability trap. | Renamed entity to `TaskItem` (kept) and enum to `WorkItemStatus`. |
